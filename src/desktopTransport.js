...
// (wrapper-plan.md §7 W6). This module never constructs the path: it hands
// the Dropbox ROOT across the bridge and wrapper/electron/syncPath.js works
// out the rest, which is what makes one file readable by both shells. ⚠ Two
// halves of that path have each bitten once. The app folder's real on-disk
// name is NOT "OELA" (wrapper-plan.md §6 trap 10, found live against the
// real account), and its PARENT is not reliably "Apps" either — Dropbox
// localizes that one, so syncPath.js looks for the app folder rather than
// assuming where it sits. Both failures were silent: a wrong path means the
// two devices sync happily with different files and never say so.
//
