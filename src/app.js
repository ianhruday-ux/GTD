  ...
  function screenQuickAdd(destKind, title){
    ...
    // Duplicate-title check (§7 ruling, chunk 3): the fast path enforces the
    // same per-lane, case-insensitive block the drafting page does at Save.
    // There's no popup channel here, so flash the input's border red and KEEP
    // the typed text (no create, no re-render) — the user edits and presses
    // Enter again. The Waiting path above already routes through the drafting
    // page, which runs the check on Save. (Group headers are excluded, matching
    // saveScreen's own check.)
    const dup = state.tasks.next.some(function(t){
      return !t.isGroup && (t.title || "").trim().toLowerCase() === title.toLowerCase();
    });
    if (dup){
  ...
  function injectQAChecklist(){
    const FLAG = "gtd_qa_checklist_chunk3";
    ...
    addGroupWithItems("\u2705 QA \u2014 Chunk 3: Contexts (shared by Next & Waiting)", [
      { title: "A context you make on Next Actions is also there to pick on Waiting On", notes: "On Next Actions, tap +, choose \u2018New context\u2019, and name one (say \u2018Phone\u2019). Now open a Waiting On item and tap its Context box \u2014 \u2018Phone\u2019 should be in the list. Next and Waiting share one set of contexts." },
      { title: "Moving a waiting item into Next keeps it in the same context", notes: "Put a Waiting On item into a context, then promote it to Next Actions (the \u2190 arrow on its card). It should land under the same-named context on Next Actions, not lose its grouping." },
      { title: "The Context box on an item's page only lets you PICK a context, never make one", notes: "Open any Next or Waiting item and tap its Context box. You should see your existing contexts to choose from and NO \u2018+ new\u2019 option. If you have no contexts yet, it should tell you to make one with + on the lane." },
      { title: "You can drag an action into a context on the lane", notes: "On Next Actions, press and hold an action for about half a second and drag it onto a context group. It should join that context." },
      { title: "Deleting a context keeps its actions (nothing is lost)", notes: "Delete a context that still has actions in it (the \u00d7 on the context header). After the confirm, the context disappears but its actions survive, moving to the top of the lane with no context. Nothing should be deleted." }
    ]);

    addGroupWithItems("\u2705 QA \u2014 Chunk 3: A habit can be cued by a context", [
      { title: "The cue picker on a habit lists your contexts as well as your other habits", notes: "Open a habit, tap the cue/hook icon on its \u2018When?\u2019 row. The picker should be split into \u2018Habits\u2019 and \u2018Contexts\u2019 sections, and the wording should talk about a \u2018cue\u2019 rather than only a \u2018habit\u2019." },
      { title: "Picking a context shows a linked-cue tag on the habit \u2014 same look as hooking to another habit", notes: "Pick a context in that picker and save. The habit should show a linked-cue tag (the link icon and the context name) that looks the same as being hooked to another habit." },
      { title: "A context cue always shows up (it has no schedule)", notes: "A habit cued on a context should show that cue every day \u2014 a context isn't tied to particular weekdays the way a habit hook is." },
      { title: "Cueing a second habit on the same context warns you first", notes: "Cue one habit on a context, then try to cue a different habit on that same context. You should get the \u2018already has \u2026 hooked to it\u2019 warning, exactly like re-using a habit as a cue." },
      { title: "Deleting a context that a habit is cued on shows the cue as deleted", notes: "Cue a habit on a context, then delete that context from the lane. On the habit's page the cue should now read as deleted (orphaned) \u2014 the same as when a habit you were hooked to is deleted." },
      { title: "You still can't file a habit INTO a context", notes: "The Habits lane should have no context groups \u2014 you can cue a habit on a context, but you cannot drop a habit into one. Only Next Actions and Waiting On group by context." }
    ]);

    addGroupWithItems("\u2705 QA \u2014 Chunk 3: the simpler date model", [
      { title: "A Waiting item's \u2018waiting for\u2019 no longer offers a date", notes: "Open a Waiting On item. Its \u2018waiting for\u2019 should only offer free text or a linked item to wait on \u2014 there should be no option to wait for a date." },
      { title: "The deadline box has no repeat option and no \u2018make this a habit?\u2019 prompt", notes: "Give a Next Action or project a deadline. There should be no repeat/recurring dropdown on the deadline, and no bubble asking if you want to make it a habit." },
      { title: "A Next Action with a deadline can't be turned into a Waiting action", notes: "Open a Next Action, give it a deadline. The \u2018Make Waiting Action\u2019 button should be greyed out with a note that a waiting action can't hold a date. Clear the deadline and it should work again." }
    ]);

    addGroupWithItems("\u2705 QA \u2014 Chunk 3: No duplicate titles from the quick-add", [
      { title: "Adding an action from a project can't reuse a name that already exists", notes: "Open a Current Project and use its quick-add row (the \u2018Next action\u2026\u2019 box) to type a name that already exists somewhere in Next Actions, then press Enter. The box should flash with a dashed red outline, keep what you typed, and NOT create a duplicate. Change the name and it should add normally. (Capitalisation doesn't matter \u2014 \u2018Email Sarah\u2019 and \u2018email sarah\u2019 count as the same.)" }
    ]);
