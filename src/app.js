    ...
    if (isHabit && habitRun.paused){
      ...
      if (liveHooks.length){
        liveHooks.forEach(function(hk){
          if (hk.ctx){
            // Context-cue: always-live, shown as the context name with a
            // location glyph (distinct from 🔗 "After X" and 🕐 text cues).
  ...
  function screenPickHook(targetId, isCtx){
    ...
    if (isCtx){
      // Context-cue (§4.5): a context is a SHARED cue target — multiple
      // habits cueing one context is the ordinary case (§11.4), so there is
      // no "already hooked by N / Replace" takeover prompt (that guards the
      // near-exclusive habit anchor). Only the shared 7-incoming cap and
      // de-duplication gate it; the outgoing cap is the cue-row count.
      d.hookPicker = false;
      const ctx = findContext(targetId);
      const addable = !already && row && ctx && hooks.length < MAX_HOOKS &&
        habitIncomingHookCount(targetId, s.taskId) < MAX_HOOKS;
      if (!addable){ renderScreen(); return; }
      d.cueRows[d.hookPickerRow] = { hook: { id: targetId, label: ctx.name, ctx: true } };
