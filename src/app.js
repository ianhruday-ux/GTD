  ...
  function restoreTask(kind, taskId){
    ...
    delete task.completedByProject; // it is live again; the association is over
    ...
    if (kind === "current" || kind === "future"){
      ...
      // ⚠ EVENTS BEFORE NEXT ACTIONS, and the order is load-bearing: restoring a
      // pseudo-action looks its event up (onPseudoActionRestored → findEvent) to
      // roll the series back, and an event still sitting in the archive cannot
      // be found, so the row would be refused for the wrong reason.
      restoreEventsForProject(taskId);          // renders next + waiting itself
      restoreNextActionsForProject(taskId);
    }
  }
  ...
  // ⚑ THE THIRD KIND (author, 2026-08-01, extending the same ruling): "it should
  // extend to everything linked to the project, including next actions."
  //
  // Linked NEXT actions are not archived like the waiting ones — completeProject
  // completes them outright, because §4.6 says they complete silently and they
  // are genuinely done. So un-completing has to know which completed actions
  // THIS project's completion completed. Nothing recorded that, so completion
  // now stamps `completedByProject` and this reads it back.
  //
  // ⚠ Why a marker rather than "un-complete every completed action linked to
  // this project": the second one also resurrects actions the user ticked off by
  // hand days earlier, which turns an undo into a rewrite of their history. The
  // undo has to reverse the click, not the week.
  //
  // Restoration goes through restoreTask itself rather than reimplementing the
  // move, which keeps three behaviours in one place instead of two: parentage
  // sanitising, the promoted-dependents pushback (§10), and — the one that
  // matters here — a pseudo-action's undo window (§4.15c). A repeat's row comes
  // back only if the project is un-completed inside that window; outside it the
  // series has already rolled and restoring would duplicate the rolled row.
  // Fat-fingering a project and undoing it IS the inside-the-window case, which
  // is the whole subject of the ruling.
  function restoreNextActionsForProject(projectId){
    const ids = state.completed.next
      .filter(function(t){ return t.completedByProject === projectId; })
      .map(function(t){ return t.id; });
    if (!ids.length) return 0;
    let restored = 0;
    ids.forEach(function(id){
      const before = state.completed.next.length;
      restoreTask("next", id);
      if (state.completed.next.length < before) restored++;
    });
    // Whatever refused to come back (a repeat whose series has rolled on) is no
    // longer associated with a completed project at all, so the marker goes
    // either way rather than lying in wait for the next time this one is ticked.
    let dirty = false;
    state.completed.next.forEach(function(t){
      if (t.completedByProject === projectId){ delete t.completedByProject; dirty = true; }
    });
    if (dirty) saveCompletedLocal("next");
    return restored;
  }
  ...
  function completeProject(kind, projectId, onConfirmed){
    ...
    function doComplete(){
      ...
      // ⚑ Record that THIS completion is what completed them, so un-completing
      // the project can reverse exactly these and nothing the user ticked off
      // themselves (restoreNextActionsForProject). Stamped after the fact rather
      // than threaded through completeTask, which is called from six other
      // places that have no project to name.
      // ⚑ Schema: a new optional field on completed records, no migration —
      // real use has not begun (CLAUDE.md), and an action completed before this
      // shipped simply has no marker and stays completed, which is exactly what
      // it did yesterday.
      if (nextLinked.length){
        const justCompleted = {};
        nextLinked.forEach(function(l){ justCompleted[l.task.id] = true; });
        let dirty = false;
        state.completed.next.forEach(function(t){
          if (justCompleted[t.id] && t.completedByProject !== projectId){
            t.completedByProject = projectId; dirty = true;
          }
        });
        if (dirty) saveCompletedLocal("next");
      }
  ...
  // =========================================================
  // ⚑ THE MASTER SWITCH (author, 2026-08-01): "the dev tools should be hidden
  // and inaccessible in the final product. Leave them in just in case I want to
  // come back to this, but for now, they've served their purpose."
  //
  // So none of it is deleted — only the way in is closed. With this false:
  //   · the ⋯ → Debugging row is not rendered, and its panel cannot be reached;
  //   · every group reads as OFF however its gtddev_ key is already set, so a
  //     device that had the time-jump buttons switched on does not strand them
  //     visible with no menu left to switch them off;
  //   · devGroupOn(QA) being false makes applyQaScaffolding() SWEEP the QA
  //     checklist and the sprint chunk map out of the lanes at the next boot,
  //     rather than inject them. That is the same ruling: they were build
  //     scaffolding and the build is over. (It also clears the injectors' flag
  //     keys, so flipping this back on re-injects rather than finding the
  //     flags set and doing nothing.)
  //
  // TO BRING THEM BACK: flip this to true and rebuild. Nothing else moves.
  //
  // gtddev_enabled is the CHECK SUITE's way in — eleven files drive the
  // time-jump buttons, and the same escape-hatch shape already exists for sync
  // (__oelaSyncForceEnabled). Nothing in the app writes this key; it can only
  // be set by hand from a console, so it does not reopen the door above.
  // =========================================================
  const DEV_TOOLS_BUILT_IN = false;
  function devToolsEnabled(){
    return DEV_TOOLS_BUILT_IN || Storage.get("gtddev_enabled") === "1";
  }
  function devGroupOn(g){ return devToolsEnabled() && Storage.get(g.key) === "1"; }
  ...
  function settingsRootHtml(){
    ...
    return (
      ...
      // ⚑ And as of the author's 2026-08-01 ruling the row is not rendered at
      // all unless DEV_TOOLS_BUILT_IN is flipped back on — see that constant.
      (devToolsEnabled() ?
  ...
  function openSettings(){
    ...
    qs("#dialog-root .settings-menu").addEventListener("click", function(e){
      ...
      // Belt and braces: the row above is not rendered when the tools are off,
      // so this is only reachable by a stale menu still on screen when the
      // switch flips. Refusing here means there is exactly one gate, not two
      // that could drift apart.
      if (action === "settings-debug"){
        if (!devToolsEnabled()) return;
        settingsPanel = "debug"; renderSettingsMenu(); return;
      }
