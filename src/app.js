  ...
  function closeScreen(){
    // If this screen was opened *from* another screen (drafting an action
    // from a project page), fall back to that screen — with its unsaved
    // draft intact — instead of all the way out to the lanes (overnight
    // notes: creating an action from the project page returns you to the
    // project page).
  ...
  // (draft included) is stashed and restored when the new one closes,
  // whichever way it closes: save, cancel, Escape, delete, or complete.
  ...
  function injectQAChecklist(){
    ...
    addGroupWithItems("\u2705 QA \u2014 Chunk 0b: restructure + storage adapter + manifest", [
      { title: "Header reads correctly with no orphaned tagline", notes: "Header shows the GTD Console wordmark, the intray, and the calendar icon \u2014 but no \u2018Runs on its own\u2019 line under the wordmark, and no gap where it used to be." },
      { title: "App behaves identically to before the stapler move", notes: "Click through all five lanes, create/edit/complete/delete an item in each, drag-reorder a card. Nothing should look, feel, or behave differently than the pre-0b build \u2014 this chunk is a pure refactor (src/ + build.py \u2192 dist/index.html)." },
      { title: "Installable as a standalone app", notes: "On a phone: browser menu \u2192 \u2018Add to Home Screen\u2019 (or the install prompt). Confirm it installs, launches in its own window with no browser chrome, opens in portrait, and the icon + splash use the desk brown (#171513)." },
      { title: "No console errors during normal use", notes: "Open DevTools \u2192 Console while creating, editing, completing, and resetting. All localStorage access now runs through src/storage.js \u2014 there should be no uncaught exceptions, quota or otherwise." },
      { title: "Still no service worker", notes: "DevTools \u2192 Application \u2192 Service Workers is empty, and reloading always fetches the live file (no stale-cache behavior). The service worker is chunk 9, not this one." }
    ]);
