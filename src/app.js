  ...
  //
  // A staged create is deliberately NOT covered: its row carries no ✕ of its
  // own, so Delete is the only way to take back a mis-added action, and
  // withholding it would strand the row until you discarded the whole page.
  // Once saved it becomes an ordinary linked item and loses Delete on the next
  // open, which is the direction the ruling wants.
  function openedAsProjectMember(s){
    if (!s || !s.staging || !s.draft) return false;
    if (s.draft.linkedProjectId !== s.staging.projectId) return false;
  ...
  function screenBodyHtml(s){
    ...
    if ((kind === "next" || kind === "waiting" || isProjectKind(kind)) && s.taskId && !openedAsProjectMember(s)){
  ...
  function reviewCardHtml(l, s){
    ...
    if (l.kind === "pastdue" && l.pseudo){
      ...
      if (form && form.type === "text"){
        ...
        menuHtml =
          reviewBandHtml(
            // Author: above the make-something-new buttons, because attaching
            // what already exists is the first thing to try.
