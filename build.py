...
def build_fonts():
    ...
    # ⚑ The notice below is the OFL's actual requirement, not decoration. This
    # used to read "see src/assets/fonts/README.md" — a pointer to a file that
    # does not travel with the product. dist/index.html is handed to people on
    # its own, zipped into the Windows build and wrapped into the APK, and in
    # none of those does src/ come along. OFL 1.1 requires the copyright notice
    # and licence to accompany the redistributed fonts, so it has to be inline.
    # (Poly Haven's textures are CC0 and require nothing; they are credited in
    # textures.js because the work deserves it, not because it is owed.)
    rules = ["/* Vendored webfonts — generated by build.py from src/assets/fonts/.",
             ...
             "   Version 1.1 — https://openfontlicense.org/ — which permits bundling",
             "   and redistribution provided this notice travels with them and they",
             "   are not sold on their own. None are modified here beyond Google",
             "   Fonts' own latin subsetting. */"]
    for face in faces:
